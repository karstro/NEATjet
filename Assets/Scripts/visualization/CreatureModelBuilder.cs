...
namespace NeatJet.Scripts.Visualization
{
    public class CreatureModelBuilder
    {
        ...
        {
            var JetWidth = 2 * model.JetRadius;
            var JetName = "Jet" + jetIndex.ToString();
        ...
        {
            for (var jetIndex = 0; jetIndex < model.Jets; jetIndex++)
            {
                var armRotation = CreatureInternals.CalculateJetArmRotation(jetIndex, model.Jets);
                var jetStart = armRotation * model.JetArm;
